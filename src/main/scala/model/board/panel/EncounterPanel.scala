...
class EncounterPanel(characters: ArrayBuffer[Character],
                     nextPanels: ArrayBuffer[Panel])
  extends AbstractPanel(characters, nextPanels) with Equals {

  /// Documentation inherited from [[Equals]]
  override def canEqual(that: Any): Boolean = that.isInstanceOf[EncounterPanel]

  /// Documentation inherited from [[Equals]]
  override def equals(that: Any): Boolean = {
    if (canEqual(that)) {
      val other = that.asInstanceOf[EncounterPanel]
  ...
  override def hashCode: Int = {
    Objects.hash(classOf[EncounterPanel], characters, nextPanels)
  }
