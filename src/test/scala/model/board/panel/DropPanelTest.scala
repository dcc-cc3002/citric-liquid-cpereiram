...
class DropPanelTest extends munit.FunSuite {
  ...
  override def beforeEach(context: BeforeEach): Unit = {
    ...
    panel1 = new DropPanel(characters1, nextPanels1)
    panel2 = new DropPanel(characters2, nextPanels2)
  }

  test("A DropPanel has params") {
  ...
  test("A panel can be equal to another one.") {
    ...
    assertEquals(panel1, new DropPanel(characters1, nextPanels1))
  ...
  test("The hash code of a DropPanel is consistent with equals") {
    assertEquals(new DropPanel(characters1, nextPanels1).##, panel1.##)
  }

  test("A DropPanel can add and remove a character") {
    panel1.addCharacter(character1)
  ...
  test("A DropPanel can't remove a character that is not in it") {
    panel1.addCharacter(character1)
    panel1.removeCharacter(character2)
    assertEquals(panel1.characters, ArrayBuffer[Character](character1))
  }

  test("A DropPanel can add a nextPanel") {
  ...
  test("A DropPanel can remove a nextPanel") {
    panel1.addNextPanel(panel2)
    panel1.addNextPanel(panel1)
  ...
  test("A DropPanel can apply an effect (None) on a player") {
