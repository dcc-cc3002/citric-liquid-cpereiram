...
class NeutralPanelTest extends munit.FunSuite {
  ...
  test("A panel can be equal to another one.") {
    ...
    assertEquals(panel1, new NeutralPanel(characters1, nextPanels1))
  ...
  test("The hash code of a NeutralPanel is consistent with equals") {
    assertEquals(new NeutralPanel(characters1, nextPanels1).##, panel1.##)
  }

  test("A NeutralPanel can add and remove a character") {
    panel1.addCharacter(character1)
  ...
  test("A NeutralPanel can't remove a character that is not in it") {
    panel1.addCharacter(character1)
    panel1.removeCharacter(character2)
    assertEquals(panel1.characters, ArrayBuffer[Character](character1))
  }

  test("A NeutralPanel can add a nextPanel") {
  ...
  test("A NeutralPanel can remove a nextPanel") {
    panel1.addNextPanel(panel2)
    panel1.addNextPanel(panel1)
  ...
  test("A NeutralPanel can apply an effect (None) on a player") {
